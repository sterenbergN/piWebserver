...
export default function WorkoutTypeEditor() {
  ...
  const renderTypeEditor = (inline = false, formKey = 'new') => (
    <div ref={setTypeFormRef(formKey)} className="animate-fade-in" style={{ background: 'var(--background)', padding: '1rem', borderRadius: '12px', border: inline ? '1px solid var(--accent)' : '1px solid var(--accent)' }}>
  ...
  return (
    <div style={{ padding: '1.5rem' }}>
            ...
            <div key={t.id} style={{ background: 'var(--background)', padding: '1rem', borderRadius: '12px', border: '1px solid var(--surface-border)' }}>
              <div className="workout-flex-between">
                 ...
                 <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button style={{ background:'none', border:'none', color:'var(--accent)'}} onClick={() => {
                      if (addingType && editingTypeId === t.id) {
                        resetTypeEditor();
                        return;
                      }
                      setNewType(t);
       ...
       {!addingType && (
         <button className="workout-btn-primary" onClick={() => { setNewType({ name: '', muscles: [], intensity: 75, sets: 4, minReps: 8, maxReps: 12 }); setAddingType(true); setEditingTypeId(null); }}>+ Create Template</button>
       )}
